...
export default function Caixa() {
    ...
    async function fechar(evento: FormEvent) {
        ...
        await executar(async () => { await fecharSessaoCaixa(sessaoAtual.caixaId, sessaoId, { saldoFinal: final }); setModalFechamento(false); setSaldo(""); }, "Não foi possível fechar o Caixa.");
    }
    ...
    return <PageContainer>
        ...
        ]} linhas={historico} carregando={carregando} obterChaveLinha={(sessao) => sessao.sessaoId} minWidth={1100} linhaCliqueavel onLinhaClick={setDetalhe} vazio={{ titulo: "Nenhuma sessão encerrada", descricao: "As sessões fechadas aparecerão aqui." }} /></Box>}
        ...
        <Dialog open={modalFechamento} fullWidth maxWidth="xs" aria-labelledby="caixa-fechamento-titulo" onClose={() => !processando && setModalFechamento(false)}><form onSubmit={fechar}><DialogTitle id="caixa-fechamento-titulo">Fechar Caixa</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>{erroModal && <Alert severity="error">{erroModal}</Alert>}<Typography color="text.secondary">Dinheiro esperado: <strong>{dinheiro(resumo?.saldoEsperadoDinheiro)}</strong></Typography><TextField required autoFocus name="saldoFinal" autoComplete="off" label="Dinheiro informado (R$)" value={saldo} onChange={(e) => setSaldo(e.target.value)} slotProps={{ htmlInput: { inputMode: "decimal" } }} /></Stack></DialogContent><DialogActions><Button type="button" onClick={() => setModalFechamento(false)}>Cancelar</Button><Button type="submit" variant="contained" disabled={processando}>{processando ? "Fechando…" : "Fechar Caixa"}</Button></DialogActions></form></Dialog>
